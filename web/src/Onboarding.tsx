...
import {
  ...
  BANNER_COLORS, SIGILS, ITEM_NAME,
} from './engine';
import type { LordType, LordProfile, ItemType, StateId } from './engine';

const LORDS: LordType[] = ['sage', 'hero', 'wise'];

/** §4 新手引導精靈：主公創建 → 賢者問答 → 確認，產出 LordProfile。 */
export default function Onboarding({ onDone }: { onDone: (p: LordProfile) => void }) {
  ...
  const profile = useMemo(() => newLordProfile(type, birth, color, sigil, items), [type, birth, color, sigil, items]);
